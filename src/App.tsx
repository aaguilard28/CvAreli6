...
const Navigation = ({ activeSection, onNavigate, isMobileMenuOpen, toggleMobileMenu }) => {
 ...
 return (
 <nav className="fixed lg:left-0 top-0 w-full lg:w-80 h-16 lg:h-screen bg-[#1e2a38] text-gray-200
 shadow-2xl z-50">
 <div className="container mx-auto px-4 lg:px-0 h-full flex items-center justify-between
 lg:block">
 ...
 <button onClick={toggleMobileMenu} className="text-gray-200 hover:text-gray-400
 focus:outline-none">
 ...
 <div className={`fixed inset-x-0 top-16 bg-[#1e2a38] lg:static lg:block lg:h-auto lg:mt-8
 ...
 className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors duration
 200 ${
...
const CollapsibleExperience = ({ date, title, company, location, description, icon }) => {
 ...
 return (
 <div className="bg-white rounded-xl shadow-md overflow-hidden mb-4 border border-gray
 200">
 <button
 onClick={toggleOpen}
 ...
 <svg
 className={`w-6 h-6 transform transition-transform duration-300 text-[#4a688b] ${isOpen ?
 'rotate-180' : ''}`}
 fill="none"
 stroke="currentColor"
 viewBox="0 0 24 24"
...
const LanguageCard = ({ language, proficiency }) => {
 return (
 <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 border border-gray-200 flex-1 min-w-
 [150px] transition-all duration-300 hover:shadow-lg">
...
const ContactCard = ({ icon, label, value, href }) => {
 const isLink = !!href;
 const content = (
 <div className={`bg-white rounded-xl shadow-md p-4 sm:p-6 mb-4 border border-gray-200 transition
 all duration-300 ${isLink ? 'bg-gray-100 shadow-lg' : ''}`}>
...
const App = () => {
 ...
 <Section ref={(el) => (sectionRefs.current.habilidades = el)} id="habilidades" title="Habilidades
 Destacadas">
 <div className="space-y-6">
 <SkillsCard title="Experiencia Ejecutiva" icon={<Briefcase size={24} />}
 iconColor="#d97706">
 <p className="text-gray-700">Más de 15 años de experiencia realizando gestiones
 administrativas clave a nivel ejecutivo para la alta dirección.</p>
 </SkillsCard>
 <SkillsCard title="Habilidades de Gestión Gerencial" icon={<LayoutDashboard size={24} />}
 iconColor="#d97706">
 ...
 <p className="text-gray-700 mb-4">- Desliza el cursor sobre cada competencia para
 conocer más detalles.</p>
 <div className="flex flex-wrap gap-2">
 {Object.entries(portfolioData.skills.tooltips).map(([label, tooltip], idx) => (
 <div key={idx} className="relative group">
 <span className="bg-gray-200 text-gray-700 px-3 py-1 rounded-full text-sm cursor
 help border border-gray-300 group-hover:shadow-md transition">
 {label}
 <Info size={12} className="inline-block ml-1 text-gray-500 opacity-0 group
 hover:opacity-100 transition-opacity" />
 </span>
 <div className="absolute z-10 hidden group-hover:block bg-[#a8c0d9] text-gray-900
 font-medium text-xs p-3 shadow-xl rounded-md w-64 top-full mt-1 left-1/2 -translate-x-1/2">
 {tooltip}
 </div>
 </div>
 ))}
 ...
 <SkillsCard title="Actitud Personal" icon={<HeartHandshake size={24} />}
 iconColor="#d97706">
 <p className="text-gray-700">Actitud empática, asertiva y positiva.</p>
 </SkillsCard>
 </div>
 </Section>
 <Section ref={(el) => (sectionRefs.current.experiencia = el)} id="experiencia" title="Experiencia
 Profesional">
 ...
 <Section ref={(el) => (sectionRefs.current.proyectos = el)} id="proyectos" title="Proyectos de
 ...
 <Section ref={(el) => (sectionRefs.current.educacion = el)} id="educacion" title="Educación
 Académica">
